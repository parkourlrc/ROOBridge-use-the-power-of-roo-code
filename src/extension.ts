...
import * as https from 'https'; // Import https for making requests
import { URL } from 'url'; // Import URL for parsing callback URLs
import express from 'express';
import * as fs from 'fs'; // Import fs for file logging
import * as path from 'path'; // Import path for log file path construction
import { RooCodeAPI } from './rooCodeApiInterface'; // We'll define this interface based on Roo Code's API
import { inspect } from 'util'; // Import inspect for better object logging
...
function logObjectToFile(prefix: string, obj: any) {
     try {
        const timestamp = new Date().toISOString();
        let logString: string;
        try {
            // Use inspect for better details than JSON.stringify for complex objects/errors
            logString = inspect(obj, { depth: 5 }); // Limit depth slightly
        } catch (e) {
            logString = `[Error inspecting object: ${(e as Error).message}]`;
        }
...
let rooApi: RooCodeAPI | undefined = undefined;
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try { // Wrap main activation logic
        ...
        const rooCodeExtension = vscode.extensions.getExtension<RooCodeAPI>(ROO_CODE_EXTENSION_ID);
        ...
        if (rooApi && typeof rooApi.on === 'function') {
            try { // Wrap event subscription
                ...
                rooApi.on('message', async (data: any) => { // Use 'any' for now to inspect
                ...
                rooApi.on('taskCompleted', async (taskId: string, usage: any) => { // Added async
                ...
                rooApi.on('taskAborted', (taskId: string) => {
...
// Helper function to define the RooCodeAPI interface based on src/exports/api.ts
// We need to manually define the methods we intend to use.
// This avoids a direct dependency but requires manual synchronization if the API changes.
declare module './rooCodeApiInterface' {
    interface RooCodeAPI {
        ...
        // Add EventEmitter 'on' method signature if not implicitly inherited
        // Use 'any' here to reflect the uncertainty from the logs
        on(event: 'message', listener: (data: any) => void): this;
        on(event: 'taskCompleted', listener: (taskId: string, usage: any) => void): this;
        on(event: 'taskAborted', listener: (taskId: string) => void): this;
        // Add other methods/events from Roo Code's API if needed
    }
}